package external_refs_recursive
...
func TestExternalRefsRecursive(t *testing.T) {
	schema, err := loader.FromFile("schema.yaml")
	if err != nil {
		t.Fatalf("failed to load schema: %v", err)
	}

	files, err := schemancer.Generate(schema, generators.GlobalOptions{}, golang.WithPackageName("external_refs_recursive"))
	if err != nil {
