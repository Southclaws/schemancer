package external_refs_only_defs
...
func TestExternalRefsOnlyDefs(t *testing.T) {
	schema, err := loader.FromFile("schema.yaml")
	if err != nil {
		t.Fatalf("failed to load schema: %v", err)
	}

	files, err := schemancer.Generate(schema, generators.GlobalOptions{}, golang.WithPackageName("external_refs_only_defs"))
	if err != nil {
